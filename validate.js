#!/usr/bin/env node
/**
 * Neon Survivors — Pre-Deploy Validator v2
 * 배포 전 자동 버그 패턴 검사기
...
const gameLines = fs.readFileSync(GAME_JS, 'utf8').split('\n');
const htmlLines = fs.readFileSync(INDEX_HTML, 'utf8').split('\n');
const gameText  = gameLines.join('\n');
const htmlText  = htmlLines.join('\n');

// ──────────────────────────────────────────────
// 검사 1: 인덱스 루프 중 사망 유발 직접 호출
// stage-clear를 유발 가능한 배열(enemies/mines/blackHoles)만 검사
// particles/gems/floatingTexts 등은 stage-clear 미유발 → 제외
// ──────────────────────────────────────────────
...
for (let i = 0; i < gameLines.length; i++) {
  ...
  if (inLoop && DANGEROUS_ARRAYS.has(loopArr)) {
    if (new RegExp(`${loopArr}\\[i\\]\\.takeDamage`).test(line))
      err('game.js', i+1, `${loopArr}[i].takeDamage() — 루프 중 스테이지클리어 유발 가능`);
    if (new RegExp(`${loopArr}\\[i\\]\\.die\\(`).test(line))
      err('game.js', i+1, `${loopArr}[i].die() — 루프 중 스테이지클리어 유발 가능`);
  }
}
...
// ──────────────────────────────────────────────
// 검사 2: 전역 배열 재할당 (= [] 패턴)
// 루프 중 전역 배열을 = []로 교체하면 기존 참조가 깨져 undefined 크래시
// 허용: 선언문(let/var/const X = []) or startGame/resetGame 함수 내부
// ──────────────────────────────────────────────
...
for (let i = 0; i < gameLines.length; i++) {
  ...
  for (const arr of GLOBAL_ARRS) {
    ...
    if (/^\s*(let|var|const)\s/.test(line)) continue; // 선언식 허용
    const ctx = gameLines.slice(Math.max(0, i-80), i).join('\n');
    if (SAFE_FN_MARKERS.some(mk => ctx.includes(mk))) continue;
    err('game.js', i+1, `${arr} = [] — 게임 진행 중 재할당 위험. .length = 0 사용 필요`);
  }
}
  ...
  if (inForOf) {
    ...
    if (inForOf && DANGEROUS_ARRAYS.has(forOfArr))
      if (new RegExp(`${forOfArr}\\.splice\\(`).test(line))
        err('game.js', i+1, `for..of ${forOfArr} 루프 중 .splice() — [...${forOfArr}] 스냅샷 필요`);
  }
}
ok('for..of 루프 splice 검사 완료');

// ──────────────────────────────────────────────
// 검사 4: 무기 update() 호출부 보호 확인
// Player.update에서 level>0 가드 있으면 개별 가드 불필요
// ──────────────────────────────────────────────
console.log('🔍 [4] 무기 update() 호출 보호 검사...');
const callProtected = gameLines.some(l =>
  /level\s*>\s*0.*update|\.level.*>\s*0.*this\.\w+\.update/.test(l)
);
if (callProtected) ok('무기 update() — 호출부에서 level>0 가드 확인');
else warn('game.js', 1251, '무기 update() 호출부에서 level>0 체크 없음 — 개별 가드 필요');
...
for (let i = 0; i < gameLines.length; i++) {
  const m = gameLines[i].match(/gameState\s*=\s*(STATE_\w+)/);
  if (m && !VALID_STATES.has(m[1]))
    err('game.js', i+1, `알 수 없는 gameState 값: ${m[1]}`);
}
ok('STATE 전환 유효성 — 안전');

// ──────────────────────────────────────────────
// 검사 6: 핵심 게임플레이 모달 닫기 후 게임루프 재시작 여부
// 설정/메타/초대 모달은 STATE 안 바꾸므로 제외
// ──────────────────────────────────────────────
...
for (const fn of GAMEPLAY_CLOSE_FNS) {
  const fnIdx = gameLines.findIndex(l => new RegExp(`function\\s+${fn}\\b`).test(l));
  if (fnIdx < 0) { warn('game.js', 0, `${fn} 함수를 찾을 수 없음`); continue; }
  let depth = 0, body = '';
  for (let i = fnIdx; i < Math.min(fnIdx+50, gameLines.length); i++) {
    const l = gameLines[i];
  ...
  if (body.includes('ensureGameLoopRunning') || /gameState\s*=\s*STATE_/.test(body))
    ...
    err('game.js', fnIdx+1, `${fn}: gameState 전환 및 ensureGameLoopRunning() 모두 없음`);
}

// ──────────────────────────────────────────────
// 검사 7: activeBoss 직접 접근 안전성
// 허용: e === activeBoss 조건 내, activeBoss &&, activeBoss?.
// ──────────────────────────────────────────────
...
for (let i = 0; i < gameLines.length; i++) {
  ...
  // 같은 줄 또는 앞 2줄에 안전 패턴 있으면 허용
  // 허용: 같은 줄에 if(activeBoss), e===activeBoss, &&, ?.
  if (/if\s*\(\s*activeBoss|e\s*===\s*activeBoss|activeBoss\s*&&|activeBoss\s*\?\./.test(line)) continue;
  // 허용: 앞 8줄 안에 guard 있음 (&& activeBoss 포함)
  const prev8 = gameLines.slice(Math.max(0, i-8), i).join('\n');
  if (/if\s*\(\s*activeBoss|e\s*===\s*activeBoss|activeBoss\s*&&|&&\s*activeBoss/.test(prev8)) continue;
  bossUnsafe++;
  if (bossUnsafe <= 3) warn('game.js', i+1, `activeBoss.* — null 체크 없음`);
}
if (bossUnsafe === 0) ok('activeBoss 접근 — 안전');
else if (bossUnsafe > 3) warn('game.js', 0, `activeBoss 미보호 접근 추가 ${bossUnsafe-3}곳`);

// ──────────────────────────────────────────────
// 검사 8: HTML ID 참조 검사
// 동적으로 .id = 'X' 로 생성되는 요소는 false positive 제외
// ──────────────────────────────────────────────
console.log('🔍 [8] HTML ID 참조 검사...');
const referencedIds = new Map();
for (const m of [...gameText.matchAll(/getElementById\(['"]([^'"]+)['"]\)/g)]) {
  if (!referencedIds.has(m[1]))
    referencedIds.set(m[1], gameText.substring(0, m.index).split('\n').length);
}
const dynamicIds = new Set([...gameText.matchAll(/\.id\s*=\s*['"]([^'"]+)['"]/g)].map(m => m[1]));
...
for (const [id, line] of referencedIds) {
  ...
  err('index.html', line, `#${id} — HTML에 없고 동적 생성도 없음`);
  missingHtml++;
}
...
try {
  ...
  err('game.js', 0, `문법 오류: ${(e.stderr||'').toString().trim() || e.message}`);
}
...
for (let i = 0; i < gameLines.length; i++) {
  ...
  if (stPos >= 0) {
    ...
    if (stDepth2 <= 0 && i > stPos) {
      const block = gameLines.slice(stPos, i+1).join('\n');
      if (/player\.[a-zA-Z]/.test(block) && !/if\s*\(!?\s*player/.test(block) && !/player\?\./.test(block)) {
        stUnsafe++;
        if (stUnsafe <= 3) warn('game.js', stPos+1, `setTimeout 내 player.* 접근 — null 체크 권장`);
      }
      stPos = -1;
    }
  }
...
else if (stUnsafe > 3) warn('game.js', 0, `setTimeout player 미보호 추가 ${stUnsafe-3}곳`);

// ──────────────────────────────────────────────
// 검사 11: mines/blackHoles 루프 경계 체크 (이전 크래시 재발 방지)
// ──────────────────────────────────────────────
console.log('🔍 [11] mines/blackHoles 루프 경계 체크 검사...');
const mineLoopIdx = gameLines.findIndex(l => /for.*mines\.length.*-.*1/.test(l));
if (mineLoopIdx >= 0) {
  const block = gameLines.slice(mineLoopIdx, mineLoopIdx+10).join('\n');
  if (/i\s*>=\s*mines\.length/.test(block)) ok('mines 루프 — 경계 체크 있음');
  else err('game.js', mineLoopIdx+1, 'mines 루프 경계 체크 없음 (stage-clear 크래시 위험)');
}
const bhLoopIdx = gameLines.findIndex(l => /for.*blackHoles\.length.*-.*1/.test(l));
if (bhLoopIdx >= 0) {
  const block = gameLines.slice(bhLoopIdx, bhLoopIdx+10).join('\n');
  if (/i\s*>=\s*blackHoles\.length/.test(block)) ok('blackHoles 루프 — 경계 체크 있음');
  else err('game.js', bhLoopIdx+1, 'blackHoles 루프 경계 체크 없음 (stage-clear 크래시 위험)');
}
...
const gameKB = Math.round(fs.statSync(GAME_JS).size / 1024);
const htmlKB = Math.round(fs.statSync(INDEX_HTML).size / 1024);
if (gameKB > 800) warn('game.js', 0, `${gameKB}KB — 분할 고려`); else ok(`game.js ${gameKB}KB`);
if (htmlKB > 100) warn('index.html', 0, `${htmlKB}KB — 분할 고려`); else ok(`index.html ${htmlKB}KB`);
...
let todoCount = 0;
for (let i = 0; i < gameLines.length; i++) {
  if (/\/\/\s*(TODO|FIXME|HACK|XXX|BUG)\b/i.test(gameLines[i])) {
    todoCount++;
    if (STRICT) warn('game.js', i+1, `미완성 주석: ${gameLines[i].trim()}`);
  }
}
